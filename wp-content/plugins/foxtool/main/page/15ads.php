...
<div id="play15" class="ft-card toggle-div">
  <h3><i class="fa-regular fa-rectangle-ad"></i> <?php _e('Ads appear when clicking on the website', 'foxtool') ?></h3>
	...
	<select style="width:120px;" name="foxtool_settings[ads-click-c3]"> 
	<?php foreach($styles as $style) { ?> 
	<?php if(isset($foxtool_options['ads-click-c3']) && $foxtool_options['ads-click-c3'] == $style) { $selected = 'selected="selected"'; } else { $selected = ''; } ?>
	<option value="<?php echo $style; ?>" <?php echo $selected; ?>><?php echo $style; ?></option> 
	<?php } ?> 
	</select>
	<label class="ft-label-right"><?php _e('Action taken', 'foxtool'); ?></label>
	</p>
	<p>
